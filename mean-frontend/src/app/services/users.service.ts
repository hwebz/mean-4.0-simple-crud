...
export class UserService {
    public baseURL: String = 'http://localhost:3500/'
    ...
    getUsers(searchCriteria: any): Observable<User[]> {
      ...
      return this.http.get(this.baseURL + 'getUsers', { search: params })
        .map((res: any) => {
          return res.json()
        })
        .catch((error: any) => {
    ...
    deleteUser(user: User): Observable<any> {
      return this.http.post(this.baseURL + 'deleteUser', { id: user._id })
        .map((res: any) => {
          return res.json()
        })
        .catch((error: any) => {
